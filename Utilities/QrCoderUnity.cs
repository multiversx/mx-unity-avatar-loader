...
namespace QRCoder.Unity
{
    public class UnityQrCode : AbstractQRCode
    {
        ...
        {
            return this.GetGraphic(pixelsPerModule, Color.black, Constants.AccentColor);
        }
        ...
        )
        {
            return this.GetGraphic(
                pixelsPerModule,
                HexToColor(darkColorHtmlHex),
                HexToColor(lightColorHtmlHex)
            );
        }
        ...
        public static Color HexToColor(string hexColor)
        {
            ...
            var b = byte.Parse(
                hexColor.Substring(4, 2),
                System.Globalization.NumberStyles.HexNumber
            );
            if (hexColor.Length == 8)
            {
                a = byte.Parse(
                    hexColor.Substring(6, 2),
                    System.Globalization.NumberStyles.HexNumber
                );
            }
        ...
        public Texture2D GetGraphic(int pixelsPerModule, Color darkColor, Color lightColor)
        {
            ...
            for (var x = 0; x < size; x = x + pixelsPerModule)
            {
                ...
                {
                    var module = this.QrCodeData.ModuleMatrix[
